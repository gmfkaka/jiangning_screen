<template>
  <div class="position-relative">
    ...
    <current-location v-if="type === 'm2o-plus'"></current-location>
    <tainei-inform v-else></tainei-inform>
  </div>
</template>
<script>
import currentLocation from '../currentLocation/currentLocation'
import taineiInform from '../taineiInform/taineiInform'
...
export default {
  ...
  created () {
    ...
    // 定时器循环
    setInterval(() => {
      this.times = storage.get('characterTwo')
      // 每天8:0定时刷新
      if (getNowTime() === '8:0') {
      ...
      if ((this.endTime - this.startTime) > 60000) {
        if (this.times > 0) {
          // times次数减少
          this.times --
          storage.set('characterTwo', this.times)
          // 发送websocket断连通知
