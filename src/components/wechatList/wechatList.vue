<template>
    <div class="main-wrap">
        <div class="click-wrap">
            <div class="text">短视频榜</div>
            <div class="click-title overhidden">
                <div class="click-list-wrap">
                    <div class="article-list sys-flex sys-flex-center animated" v-for="(item,k) in reportList" v-bind:key="k" :class="{'fadeInLeft' : item.title}" v-bind:style="{'animation-delay' : k/2+'s'}">
                        <!-- <div class="status dahui" v-if="item.status == 1 ">已打回</div>
                        <div class="status daishenhe" v-if="item.status == 2 ">待审核</div>
                        <div class="status tongguo"  v-if="item.status == 3 ">已通过</div> -->
                        ...
                        <div class="article-title overhidden">{{item.title}}</div>
                        <div class="user">{{item.author}}</div>
                        <div class="time">{{item.publish_time.slice(5,10)}}</div>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        getReportList(){
            ...
            this.$api.getWechatNewsList().then((response)=>{
        ...
        initList(){
            this.reportList = this.list.slice( this.count , this.count + 6 > this.list.length ? this.list.length : this.count + 6 );
            this.count += 6;
