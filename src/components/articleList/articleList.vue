<template>
  <div class="main-wrap">
    <div class="click-wrap">
      <div class="text">发布稿件</div>
      <div class="click-title overhidden">
        <div class="click-list-wrap">
          <div
            class="article-list sys-flex sys-flex-center animated"
            v-for="(item,k) in reportList"
            v-bind:key="k"
            :class="{'fadeInLeft' : item.title}"
            v-bind:style="{'animation-delay' : k/2+'s'}"
          >
            <div class="comment-rank first" v-if="k===0">01</div>
            <div class="comment-rank second" v-if="k===1">02</div>
            <div class="comment-rank third" v-if="k===2">03</div>
            <div class="comment-rank others" v-if="k>2">0{{k+1}}</div>
            <div class="article-title">{{item.title}}</div>
            <div class="source">{{item.column_name}}</div>
            <div class="user">{{item.author}}</div>
            <div class="time" v-if="item.publish_time">{{item.publish_time.slice(5,10)}}</div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "articleList",
  data() {
    return {
      reportList: [],
      count: 0
    };
  },

  created() {
    this.getReportList();
  },

  methods: {
    getReportList() {
      if (this.countNum) {
        this.reportList = [];
        clearInterval(this.countNum);
        this.count = 0;
      }
      this.$api.getNewsList().then(response => {
        if (response.data.result.length) {
          this.list = response.data.result;
          this.initList();
        }
      });
    },

    initList() {
      this.reportList = this.list.slice(
        this.count,
        this.count + 6 > this.list.length ? this.list.length : this.count + 6
      );
      this.count += 6;
      this.countNum = setInterval(() => {
        if (this.count < this.list.length) {
          this.reportList = [];
          setTimeout(() => {
            this.reportList = this.list.slice(this.count, this.count + 6);
            this.count += 6;
          }, 100);
        } else {
          this.reportList = [];
          clearInterval(this.countNum);
          this.count = 0;
          this.getReportList();
        }
      }, 15000);
    }
  }
};
</script>

<style lang="scss" scoped>
.main-wrap {
  width: 100%;
  height: 10.8rem;
  padding: 0rem 0.3rem 0.15rem;
  position: relative;
  background: url("../../assets/img/box_bg.png") no-repeat center center;
  background-size: 100% 100%;
  .click-wrap {
    width: 100%;
    height: 100%;
    .text {
      font-size: 0.5rem;
      color: #fff;
      font-weight: 800;
      position: absolute;
      left: 50%;
      top: 7%;
      transform: translate(-50%, -50%);
    }
    .comment-rank {
      width: 0.36rem;
      height: 0.36rem;
      line-height: 0.36rem;
      font-size: 0.24rem;
      text-align: center;
      border-radius: 0.03rem;
      &.first {
        background: rgba(240, 65, 65, 1);
      }
      &.second {
        background: rgba(240, 168, 65, 1);
      }
      &.third {
        background: rgba(193, 65, 240, 1);
      }
      &.others {
        background: rgba(50, 205, 228, 1);
      }
    }
    .click-title {
      width: 100%;
      height: 100%;
      background: url("../../assets/img/title_bg.png") no-repeat top center;
      background-size: 100%;
    }
    .click-list-wrap {
      margin-top: 1.8rem;
      width: 100%;
      height: 8.8rem;
      background: url("../../assets/img/wglist_bg.png") no-repeat center center;
      background-size: 100% 100%;
      padding: 0 0.65rem;
      .article-list {
        height: 16.5%;
        border-bottom: 1px solid rgba(255, 255, 255, 0.3);
        color: #fff;
        &:last-child {
          border-bottom: 0;
        }
        .article-title {
          width: 10.5rem;
          font-size: 0.38rem;
          font-weight: 400;
          padding-left: 0.4rem;
          overflow: hidden;
          text-overflow: ellipsis;
          white-space: nowrap;
        }
        .source {
          font-size: 0.3rem;
          color: #fff;
          width: 15%;
          padding-left: 0.4rem;
        }
        .user {
          background: url("../../assets/img/commentList/name.png") no-repeat
            center left;
          background-size: 0.25rem 0.28rem;
          padding-left: 0.4rem;
          font-size: 0.3rem;
          font-family: PingFangSC-Regular;
          font-weight: 400;
          color: #fff;
          width: 12%;
        }
        .time {
          font-size: 0.3rem;
        }
      }
    }
  }
}
</style>
