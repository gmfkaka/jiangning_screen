<template>
  <div class="main-wrap position-relative">
    ...
    <div class="intelligence-briefing">
      <div class="briefing-title">形象展示</div>
...
export default {
  ...
  created () {
    ...
    if (window.location.href.indexOf('monitor-jntv.cloud.hoge.cn') > -1) {
      window.location.href = 'http://smart-jntv.cloud.hoge.cn/#/intelligenceBriefing'
    }
    // 获取宣传视频
    this.getVideo()
    // 定时器循环
      ...
      if ((this.endTime2 - this.startTime3) > 60000) {
        if (this.times2 > 0) {
          ...
          if (this.from) {
            ...
            link = 'ws://10.1.1.22:8011/websocket/receive'
          }
          let name = 'ai人像识别'
          this.$api.isWebsocket(link, name).then(res => {
            // console.log(res)
  ...
  methods: {
    initData () {
      ...
      if (this.from) {
        ...
        this.socketClient = new WebSocket('ws://10.1.1.22:8011/websocket/receive')
      }
    ...
    handleData (data) {
      ...
      if (data && data[0]) {
        ...
        console.log('handleList', this.list)
