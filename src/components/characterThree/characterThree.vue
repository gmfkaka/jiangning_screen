<template>
  <div class="position-relative">
    ...
    <profile v-if="type === 'm2o-plus'" :id="id"></profile>
    <my-mission v-else-if="type === 'workcall'" :id="id" :token="token"></my-mission>
    <comment-list v-else-if="type === 'old'"></comment-list>
  </div>
</template>
<script>
import profile from '../profile/profile'
import myMission from '../myMission/myMission'
import commentList from '../commentlList/commentList'
...
export default {
  ...
  created () {
    ...
    // 定时器循环
    setInterval(() => {
      this.times = storage.get('characterThree')
      // 每天8:0定时刷新
      if (getNowTime() === '8:0') {
      ...
      if ((this.endTime - this.startTime) > 60000) {
        if (this.times > 0) {
          // times次数减少
          this.times --
          storage.set('characterThree', this.times)
          // 发送websocket断连通知
